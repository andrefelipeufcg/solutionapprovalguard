...
function plugin_solutionapprovalguard_pre_item_add(CommonDBTM $item) {
    if ($item instanceof ITILFollowup) {
        // Verifica se é uma ação de aprovação de solução
        if (isset($item->input['_close']) && $item->input['_close'] == 1) {
            ...
            $allow_comments = 0; // Padrão: Permitido
            if (count($iterator)) {
                $allow_comments = $iterator->current()['allow_comments'];
            }

            // Se a configuração exigir ação: Aviso (1) ou Bloqueio (2)
            if ($allow_comments == 1 || $allow_comments == 2) {
                ...
                if (!empty($clean_content)) {
                    ...
                    if ($allow_comments == 2) {
                        ...
                        // A ausência de $item->input = false; permite que a gravação continue normalmente
                    }
                }
            }
        }
