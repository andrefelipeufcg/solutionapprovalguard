...
<a id="english"></a>
# Solution Approval Guard (GLPI 11 Plugin)
...
**Solution Approval Guard** is a usability and process integrity plugin developed specifically for **GLPI 11**. It allows administrators to control whether end users can enter text comments when they decide to **approve** a ticket solution.
...
- **Agnostic Request Interception:** Capable of capturing approvals made both through the traditional form (`$_POST`) and through asynchronous payloads (`application/json`) sent via AJAX by the new GLPI 11 Timeline.
...
The repository is structured strictly following the good practices and development standards of the GLPI 11 ecosystem:
...
Since GLPI 11 heavily stores plugin structures and views in cache (via Symfony/Twig), it is **highly recommended** to clear the system cache after installing or updating plugin files.
...
### Scope Hooks
The plugin uses the GLPI 11 hook ecosystem to guarantee total coverage:
